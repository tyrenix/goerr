...
func main() {
	...
	// // repository layer
	// repoErr := goerr.New("internal", "postgres connection timeout", goerr.Kind(errInternal))
	...
	// FIXME:
	main := goerr.New("main_error", goerr.Kind(errors.New("errNotFound")))
	e1 := goerr.Wrap(main, "repo any error")
	e2 := goerr.Wrap(e1, "service", goerr.Field("count", 1), goerr.Field("op", "op1"))
	e3 := goerr.Wrap(e2, "handler")
	fmt.Println("error:", e3)
	fmt.Println("short error e3:", e3.Error())
	fmt.Println("short error e2:", e2.Error())
}
